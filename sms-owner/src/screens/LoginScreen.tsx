...
export default function LoginScreen() {
  ...
  async function handleLogin() {
    if (!phone || !password) { Alert.alert('Missing info', 'Enter your phone and password'); return }
    setLoading(true)
    try {
      const res = await ownerAuthApi.login(phone, password)
      const { accessToken, owner } = res.data.data
      await setAuth(owner, accessToken)
    } catch (err: any) {
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          ...
          <Text style={styles.brandSub}>OWNER PORTAL</Text>
          <Text style={styles.heroTitle}>Your properties.{'\n'}Your income.{'\n'}<Text style={styles.heroAccent}>Your insights.</Text></Text>
          <Text style={styles.heroSub}>View occupancy, track rental income, set rent amounts, and receive monthly statements — without calling your agent.</Text>
        </View>

        <View style={styles.card}>
          <Text style={styles.cardTitle}>Property Owner Sign In</Text>
          <Text style={styles.cardSub}>Your login credentials are provided by your managing agent</Text>
          ...
          <TouchableOpacity style={[styles.btn, loading && { opacity: 0.6 }]} onPress={handleLogin} disabled={loading}>
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Sign In to Owner Portal</Text>}
          </TouchableOpacity>

          <Text style={styles.note}>Don't have login details? Your property agent will set up your account and share your credentials.</Text>
        </View>

        <Text style={styles.footer}>Waltern Tech Ltd · Property Management Platform · Nairobi, Kenya</Text>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 56, height: 56, borderRadius: 16, backgroundColor: 'rgba(139,92,246,0.15)', alignItems: 'center', justifyContent: 'center', marginBottom: 20, borderWidth: 1, borderColor: 'rgba(139,92,246,0.3)' },
  brand: { color: '#fff', fontSize: 18, fontWeight: '700', marginBottom: 2 },
  brandSub: { color: '#8B5CF6', fontSize: 10, fontWeight: '700', letterSpacing: 3, marginBottom: 24 },
  heroTitle: { color: '#fff', fontSize: 36, fontWeight: '800', lineHeight: 44, letterSpacing: -0.5, marginBottom: 12 },
  heroAccent: { color: '#8B5CF6' },
  ...
  btn: { backgroundColor: '#8B5CF6', borderRadius: 12, paddingVertical: 16, alignItems: 'center' },
